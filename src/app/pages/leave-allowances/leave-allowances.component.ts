...
import { LeaveAllowanceQueryService } from '../../core/services/leave-allowances/leave-allowance-query.service';
...
import { LeaveAllowanceFormComponent } from './leave-allowance-form.component';
...
export class LeaveAllowancesComponent implements OnInit {
  protected readonly query = inject(LeaveAllowanceQueryService);
  ...
  private readonly mutation = inject(LeaveAllowanceMutationService);
  ...
  fetchData(page: number = this.query.page()) {
    this.query
      .fetchAll(page, false, this.searchTerm, this.query.limit(), this.query.sort(), {
        ...
        year: this.yearFilter ? Number(this.yearFilter) : undefined,
      })
      .subscribe();
  }
  ...
  async openForm(leaveAllowance: LeaveAllowance | null = null) {
    const result = await this.modal.open(LeaveAllowanceFormComponent, {
      leaveAllowance,
    });
  ...
  async onDelete(id: string) {
    const ok = await this.confirm.open({
      title: 'Delete Leave Allowance',
      message: 'Are you sure you want to delete this leave allowance?',
      confirmText: 'Delete',
      cancelText: 'Cancel',
    });
    ...
    if (ok) {
      this.mutation.remove(id).subscribe({
        ...
        error: () => this.toast.error('Failed to delete leave allowance'),
      });
    }
  }
}
